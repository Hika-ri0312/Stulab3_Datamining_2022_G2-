""" 
西洋画の画像をリサイズし、pandas.DataFrame()に格納してpickle形式で保存する.
...
def resize(path, folders):
    ...
    for file in folders:
        ...
        img_gray_e = cv2.resize(img_gray,dsize=(64,64)) #=> 約 12,448 のリストの長さ
...
def get_plickle():
    ...
    for i, vecter_imgs in enumerate(x):
