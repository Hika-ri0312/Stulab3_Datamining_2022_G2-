...
def linearSVC(x,y):
    """ 外部ライブラリのsklearnより学習モデルlinearSVCによって、学習を行う。
    ...
    model (sklearn.svm.LinearSVC): LinearSVCモデル. 
    ...
    #モデルインスタンス
    model = LinearSVC(max_iter = 100, random_state=0) #グリッドサーチのみ高速で行うために反復回数を100に制限
    ...
    model = LinearSVC(  C = gridsearch.best_params_['C'], 
                        loss = gridsearch.best_params_['loss'],
                        random_state = 0,
                        )
    print(model)
